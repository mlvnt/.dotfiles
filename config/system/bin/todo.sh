...
oneline_usage="$TODO_SH [-fhpantvVz] [-d todo_config] action [task_number] [task_description]"
    ...
    do
        ...
        if [ -f "$action" -a -x "$action" ]; then
            ...
            if [ "$builtinActionUsage" ]; then
                echo "${SPACING}$builtinActionUsage"
                echo
            else
                die "${SPACING}TODO: No action \"${actionName}\" exists."
            fi
        fi
    done
}
...
getPrefix()
{
    ...
    # Returns:       Uppercase FILE prefix to be used in place of "${SPACING}TODO:" where
    #                a different todo file can be specified.
    local base=$(basename "${1:-$TODO_FILE}")
    echo "${SPACING}${base%%.[^.]*}" | tr 'a-z' 'A-Z'
}
...
getNewtodo()
{
    ...
    local item=$1
    [ -z "$item" ] && die '${SPACING}Programming error: $item should exist.'
    [ "${item//[0-9]/}" ] && die '${SPACING}Programming error: $item should be numeric.'

    newtodo=$(sed "$item!d" "${2:-$TODO_FILE}")
    [ -z "$newtodo" ] && die "$(getPrefix "$2"): Task $item not updated."
}
...
replaceOrPrepend()
{
  ...
  if [ $TODOTXT_VERBOSE -gt 0 ]; then
    ...
    case "$action" in
      replace)
        echo "${SPACING}$item $todo"
        echo "${SPACING}TODO: Replaced task with:"
        echo "${SPACING}$item $newtodo"
        ;;
      prepend)
        echo "${SPACING}$item $newtodo"
        ;;
...
# == PROCESS OPTIONS ==
while getopts ":fhpcnNaAtTvVxz+@Pd:" Option
do
  case $Option in
    '@' )
    ...
    x )
        ...
        ;;
    z )
        ## HIDE_DATE_ON_ADD starts at zero (false); increment it to one
        ##   (true) the first time this flag is seen. Each time the flag
        ##   is seen after that, increment it again so that an even
        ##   number shows priority labels and an odd number hides date on add
        : $(( HIDE_DATE_ON_ADD_LABLES++ ))
        if [ $(( $HIDE_DATE_ON_ADD_LABLES % 2 )) -eq 0 ]
        then
            ## Zero or even value -- show date on add
            unset HIDE_DATE_ON_ADD
        else
            ## One or odd value -- hide date on add
            export HIDE_DATE_ON_ADD="[[:space:]](19|20)[0-9]{2}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])"
        fi
        ;;
  esac
done
...
_list() {
    ...
    if [ "${1:0:1}" == / ]; then
        ...
        die "${SPACING}TODO: File $FILE does not exist."
    fi
...
_format()
{
    ...
    filtered_items=$(
        ...
        | awk '''
            ...
            }
            {
                ...
                if (match($0, /^[0-9]+ (19|20)[0-9]{2}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01]) /)) {
                    if (ENVIRON["HIDE_DATE_ON_ADD"] != "") {
                         $0 = substr($0, 1, RLENGTH - 11) substr($0, RSTART + RLENGTH)
                     }
                }

                if (match($0, /^[0-9]+ \([A-Z]\)+ (19|20)[0-9]{2}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01]) /)) {
                    if (ENVIRON["HIDE_DATE_ON_ADD"] != "") {
                         $0 = substr($0, 1, RLENGTH - 11) substr($0, RSTART + RLENGTH)
                     }
                }
    ...
    if [ $TODOTXT_VERBOSE -gt 1 ]; then
        echo "${SPACING}TODO DEBUG: Filter Command was: ${filter_command:-cat}"
    fi
}
...
"add" | "a")
    if [[ -z "$2" && $TODOTXT_FORCE = 0 ]]; then
        ...
        [ -z "$2" ] && die "${SPACING}usage: $TODO_SH add \"TODO ITEM\""
        shift
        input=$*
    fi
...
"addm")
    if [[ -z "$2" && $TODOTXT_FORCE = 0 ]]; then
        ...
        [ -z "$2" ] && die "${SPACING}usage: $TODO_SH addm \"TODO ITEM\""
        shift
        input=$*
    fi
...
"addto" )
    [ -z "$2" ] && die "${SPACING}usage: $TODO_SH addto DEST \"TODO ITEM\""
    dest="$TODO_DIR/$2"
    [ -z "$3" ] && die "${SPACING}usage: $TODO_SH addto DEST \"TODO ITEM\""
    shift
    shift
    input=$*
    ...
    if [ -f "$dest" ]; then
        ...
        die "${SPACING}TODO: Destination file $dest does not exist."
    fi
    ;;

"append" | "app" )
    errmsg="${SPACING}usage: $TODO_SH append ITEM# \"TEXT TO APPEND\""
    shift; item=$1; shift
    getTodo "$item"

    if [[ -z "$1" && $TODOTXT_FORCE = 0 ]]; then
        echo -n "${SPACING}Append: "
        read -e -r input
    else
        input=$*
    fi
    ...
    if sed -i.bak $item" s|^.*|&${appendspace}${input}|" "$TODO_FILE"; then
        ...
        die "${SPACING}TODO: Error appending task $item."
    fi
    ;;
...
"archive" )
    ...
    if [ $TODOTXT_VERBOSE -gt 0 ]; then
	echo "${SPACING}TODO: $TODO_FILE archived."
    fi
    ;;

"del" | "rm" )
    # replace deleted line with a blank line when TODOTXT_PRESERVE_LINE_NUMBERS is 1
    errmsg="${SPACING}usage: $TODO_SH del ITEM# [TERM]"
    item=$2
    getTodo "$item"

    if [ -z "$3" ]; then
        if  [ $TODOTXT_FORCE = 0 ]; then
            echo "${SPACING}Delete '$todo'?  (y/n)"
            read -e -r ANSWER
        else
            ANSWER="y"
        fi
        if [ "$ANSWER" = "y" ]; then
            ...
            if [ $TODOTXT_VERBOSE -gt 0 ]; then
                echo "${SPACING}$item $todo"
                echo "${SPACING}TODO: $item deleted."
            fi
        else
            echo "${SPACING}TODO: No tasks were deleted."
        fi
    else
        sed -i.bak \
            -e $item"s/^\((.) \)\{0,1\} *$3 */\1/g" \
        ...
        if [ "$todo" = "$newtodo" ]; then
            [ $TODOTXT_VERBOSE -gt 0 ] && echo "${SPACING}$item $todo"
            die "${SPACING}TODO: '$3' not found; no removal done."
        fi
        if [ $TODOTXT_VERBOSE -gt 0 ]; then
            echo "${SPACING}$item $todo"
            echo "${SPACING}TODO: Removed '$3' from task."
            echo "${SPACING}$item $newtodo"
        fi
    fi
    ;;
...
"depri" | "dp" )
    errmsg="${SPACING}usage: $TODO_SH depri ITEM#[, ITEM#, ITEM#, ...]"
    shift;
    [ $# -eq 0 ] && die "$errmsg"
	...
	if [[ "$todo" = \(?\)\ * ]]; then
		...
		getNewtodo "$item"
		echo "${SPACING}$item $newtodo"
		echo "${SPACING}TODO: $item deprioritized."
	    fi
	else
	    echo "${SPACING}TODO: $item is not prioritized."
	fi
    done
    ;;

"do" )
    errmsg="${SPACING}usage: $TODO_SH do ITEM#[, ITEM#, ITEM#, ...]"
    ...
    for item in ${*//,/ }; do
        ...
        if [ "${todo:0:2}" != "x " ]; then
            ...
            if [ $TODOTXT_VERBOSE -gt 0 ]; then
                getNewtodo "$item"
                echo "${SPACING}$item $newtodo"
                echo "${SPACING}TODO: $item marked as done."
	    fi
        else
            echo "${SPACING}TODO: $item is already marked done."
        fi
    done
...
"listall" | "lsa" )
    ...
    if [ $TODOTXT_VERBOSE -gt 0 ]; then
        ...
        echo "${SPACING}total $((TASKNUM + DONENUM)) of $((TOTAL + TDONE)) tasks shown"
    fi
    ;;
...
"listfile" | "lf" )
    ...
    if [ $# -eq 0 ]; then
        [ $TODOTXT_VERBOSE -gt 0 ] && echo "${SPACING}Files in the todo.txt directory:"
...
"move" | "mv" )
    ...
    errmsg="${SPACING}usage: $TODO_SH mv ITEM# DEST [SRC]"
    item=$2
    dest="$TODO_DIR/$3"
    src="$TODO_DIR/$4"
    ...
    [ -f "$src" ] || die "${SPACING}TODO: Source file $src does not exist."
    [ -f "$dest" ] || die "${SPACING}TODO: Destination file $dest does not exist."

    getTodo "$item" "$src"
    [ -z "$todo" ] && die "${SPACING}$item: No such item in $src."
    if  [ $TODOTXT_FORCE = 0 ]; then
        echo "${SPACING}Move '$todo' from $src to $dest? (y/n)"
        read -e -r ANSWER
    else
        ANSWER="y"
    fi
    if [ "$ANSWER" = "y" ]; then
        ...
        if [ $TODOTXT_VERBOSE -gt 0 ]; then
            echo "${SPACING}$item $todo"
            echo "${SPACING}TODO: $item moved from '$src' to '$dest'."
        fi
    else
        echo "${SPACING}TODO: No tasks moved."
    fi
    ;;

"prepend" | "prep" )
    errmsg="${SPACING}usage: $TODO_SH prepend ITEM# \"TEXT TO PREPEND\""
...
note: PRIORITY must be anywhere from A to Z."
    ...
    if [ $TODOTXT_VERBOSE -gt 0 ]; then
        ...
        if [ "$oldpri" != "$newpri" ]; then
            if [ "$oldpri" ]; then
                echo "${SPACING}TODO: $item re-prioritized from ($oldpri) to ($newpri)."
            else
                echo "${SPACING}TODO: $item prioritized ($newpri)."
            fi
        fi
    fi
    if [ "$oldpri" = "$newpri" ]; then
        echo "${SPACING}TODO: $item already prioritized ($newpri)."
    fi
    ;;

"replace" )
    errmsg="${SPACING}usage: $TODO_SH replace ITEM# \"UPDATED ITEM\""
...
"report" )
    ...
    if [ "$LASTDATA" = "$NEWDATA" ]; then
        echo "${SPACING}$LASTREPORT"
        [ $TODOTXT_VERBOSE -gt 0 ] && echo "${SPACING}TODO: Report file is up-to-date."
    else
        NEWREPORT="$(date +%Y-%m-%dT%T) ${NEWDATA}"
        echo "${NEWREPORT}" >> "$REPORT_FILE"
        echo "${SPACING}${NEWREPORT}"
        [ $TODOTXT_VERBOSE -gt 0 ] && echo "${SPACING}TODO: Report file updated."
    fi
    ;;
...
"deduplicate" )
    ...
    if [ $deduplicateNum -eq 0 ]; then
        echo "${SPACING}TODO: No duplicate tasks found"
    else
        echo "${SPACING}TODO: $deduplicateNum duplicate task(s) removed"
    fi
    ;;
